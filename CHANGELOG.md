...
* 2.7.next in progress
  * Exclude `upper-case` from `clojure.core` under `let-go`, where core defines that name, so requiring `honey.sql` no longer warns, via PR [#619](https://github.com/seancorfield/honeysql/pull/619) from [@abogoyavlensky](https://github.com/abogoyavlensky).
...
* 2.7.1479 -- 2026-09-20
  ...
  * Full support for [`let-go`](https://github.com/nooga/let-go); with tests in CI (requires let-go 1.13.0 or later and lgx 0.3.1 or later).
...
* 2.7.1392 -- 2026-06-26
  * Address [Security Advisory](https://github.com/seancorfield/honeysql/security/advisories/GHSA-fcfq-xqgp-8v97) **CVE-2026-61620**.
  ...
  * Fix escaped quote handling in `:inline` for strings to prevent SQL injection via inlining user input. Reported by [Younghun Ko](https://github.com/koyokr).
