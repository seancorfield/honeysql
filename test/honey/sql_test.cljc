...
(deftest values-alias
  (is (= ["SELECT vals.a FROM (VALUES (?, ?, ?)) AS vals (a, b, c)" 1 2 3]
         (sut/format {:select [:vals.a]
                      :from [[{:values [[1 2 3]]} [:vals {:columns [:a :b :c]}]]]}))))
(deftest test-cte
  (is (= (sut/format {:with [[:query {:select [:foo] :from [:bar]}]]})
         ["WITH query AS (SELECT foo FROM bar)"]))
  (is (= (sut/format {:with [[:query {:select [:foo] :from [:bar]} :materialized]]})
         ["WITH query AS MATERIALIZED (SELECT foo FROM bar)"]))
  (is (= (sut/format {:with [[:query {:select [:foo] :from [:bar]} :not-materialized]]})
         ["WITH query AS NOT MATERIALIZED (SELECT foo FROM bar)"]))
  (is (= (sut/format {:with [[:query {:select [:foo] :from [:bar]} :kw-1 :kw-2]]})
         ["WITH query AS (SELECT foo FROM bar) KW 1 kw_2"]))
  (is (= (sut/format {:with-recursive [[:query {:select [:foo] :from [:bar]} :cycle [:a :b :c] :set :d :to [:abs :e] :default 42 :using :x]]})
         ["WITH RECURSIVE query AS (SELECT foo FROM bar) CYCLE a, b, c SET d TO ABS(e) DEFAULT ? USING x" 42]))
  (is (= (sut/format {:with [[:query1 {:select [:foo] :from [:bar]}]
  ...
  (is (= (sut/format {:with-recursive [[:query {:select [:foo] :from [:bar]}]]})
         ["WITH RECURSIVE query AS (SELECT foo FROM bar)"]))
  (is (= (sut/format {:with [[[:static {:columns [:a :b :c]}] {:values [[1 2 3] [4 5]]}]]})
...
(deftest insert-into
  (is (= (sut/format {:insert-into :foo})
         ["INSERT INTO foo"]))
  (is (= (sut/format {:insert-into [:foo {:select [:bar] :from [:baz]}]})
         ["INSERT INTO foo SELECT bar FROM baz"]))
  (is (= (sut/format {:insert-into [[:foo [:a :b :c]] {:select [:d :e :f] :from [:baz]}]})
...
(deftest insert-into-namespaced
  ...
  (is (= (sut/format {:insert-into :foo :values [{:foo/id 1}]})
         ["INSERT INTO foo (id) VALUES (?)" 1]))
  (is (= (sut/format {:insert-into :foo :columns [:foo/id] :values [[2]]})
...
(deftest insert-into-uneven-maps
  ...
  (let [res (sut/format {:insert-into :foo :values [{:id 1} {:id 2, :bar "quux"}]})]
    (is (or (= res ["INSERT INTO foo (id, bar) VALUES (?, NULL), (?, ?)" 1 2 "quux"])
            (= res ["INSERT INTO foo (bar, id) VALUES (NULL, ?), (?, ?)" 1 "quux" 2]))))
  (let [res (sut/format {:insert-into :foo :values [{:id 1, :bar "quux"} {:id 2}]})]
...
(deftest array-test
  ...
  #_ ;; requested feature -- does not work yet
    (is (= (sut/format {:insert-into :foo
           ...
           ["INSERT INTO foo (baz) VALUES (ARRAY[?, ?, ?, ?])" 1 2 3 4]))
  (testing "typed array"
    (is (= (sut/format {:select [[[:array [] :integer]]]})
           ["SELECT ARRAY[]::INTEGER[]"]))
    (is (= (sut/format {:select [[[:array [1 2] :text]]]})
           ["SELECT ARRAY[?, ?]::TEXT[]" 1 2])))
  (testing "array subquery"
    (is (= (sut/format {:select [[[:array {:select [:foo] :from [:bar]}]]]})
           ["SELECT ARRAY(SELECT foo FROM bar)"]))
    (is (= (sut/format {:select [[[:array {:select ^{:as :struct} [:foo :bar] :from [:bar]}]]]})
...
(deftest union-test
  ...
  ;;   ORDER BY foo ASC
  (is (= (sut/format {:union [{:select [:foo] :from [:bar1]}
...
(deftest union-all-test
  (is (= (sut/format {:union-all [{:select [:foo] :from [:bar1]}
                                  {:select [:foo] :from [:bar2]}]})
         ["SELECT foo FROM bar1 UNION ALL SELECT foo FROM bar2"])))

(deftest intersect-test
  (is (= (sut/format {:intersect [{:select [:foo] :from [:bar1]}
                                  {:select [:foo] :from [:bar2]}]})
         ["SELECT foo FROM bar1 INTERSECT SELECT foo FROM bar2"])))

(deftest except-test
  (is (= (sut/format {:except [{:select [:foo] :from [:bar1]}
                               {:select [:foo] :from [:bar2]}]})
...
(deftest compare-expressions-test
  ...
  (testing "Example from dharrigan"
    (is (= ["SELECT PG_TRY_ADVISORY_LOCK(1)"]
           (sut/format {:select [:%pg_try_advisory_lock.1]}))))
...
#?(:clj
   (deftest issue-385-test
     (let [u (java.util.UUID/randomUUID)]
       (is (= [(str "VALUES ('" (str u) "')")]
              (sut/format {:values [[u]]} {:inline true}))))))
...
(deftest format-arity-test
  ...
  (testing "format can be called with an options hash map"
    (is (= ["\nDELETE FROM `foo`\nWHERE `foo`.`id` = ?\n" 42]
           (-> {:delete-from :foo
                :where [:= :foo.id 42]}
               (sut/format {:dialect :mysql :pretty true})))))
...
(deftest locking-select-tests
  (testing "PostgreSQL/ANSI FOR"
    (is (= ["SELECT * FROM foo FOR UPDATE"]
           (sut/format {:select [:*] :from :foo :for :update})))
    (is (= ["SELECT * FROM foo FOR NO KEY UPDATE"]
           (sut/format {:select [:*] :from :foo :for :no-key-update})))
    (is (= ["SELECT * FROM foo FOR SHARE"]
           (sut/format {:select [:*] :from :foo :for :share})))
    (is (= ["SELECT * FROM foo FOR KEY SHARE"]
           (sut/format {:select [:*] :from :foo :for :key-share})))
    (is (= ["SELECT * FROM foo FOR UPDATE"]
           (sut/format {:select [:*] :from :foo :for [:update]})))
    (is (= ["SELECT * FROM foo FOR NO KEY UPDATE"]
           (sut/format {:select [:*] :from :foo :for [:no-key-update]})))
...
DO NOTHING
"]
         (sut/format {:insert-into :customers
                      :columns [:name :email]
...
(deftest issue-321-linting
  (testing "empty IN is ignored by default"
    (is (= ["WHERE x IN ()"]
           (sut/format {:where [:in :x []]})))
    (is (= ["WHERE x IN ()"]
           (sut/format {:where [:in :x :?y]}
                       {:params {:y []}}))))
  ...
  (testing "empty WHERE clauses ignored with none"
    (is (= ["DELETE FROM foo"]
           (sut/format {:delete-from :foo})))
    (is (= ["DELETE foo"]
           (sut/format {:delete :foo})))
    (is (= ["UPDATE foo SET x = ?" 1]
           (sut/format {:update :foo :set {:x 1}}))))
  (testing "non-empty WHERE clauses valid in basic mode"
    (is (= ["DELETE FROM foo WHERE x = ?" 1]
           (sut/format {:delete-from :foo :where [:= :x 1]} {:checking :basic})))
    (is (= ["DELETE foo WHERE x = ?" 1]
           (sut/format {:delete :foo :where [:= :x 1]} {:checking :basic})))
    (is (= ["UPDATE foo SET x = ? WHERE x = ?" 1 1]
           (sut/format {:update :foo :set {:x 1} :where [:= :x 1]} {:checking :basic})))
    (is (= ["DELETE FROM foo WHERE x = ?" 1]
           (sut/format '{delete-from foo where (= x 1)} {:checking :basic})))
    (is (= ["DELETE foo WHERE x = ?" 1]
           (sut/format '{delete foo where (= x 1)} {:checking :basic})))
  ...
  (testing "non-empty WHERE clauses valid in strict mode"
    (is (= ["DELETE FROM foo WHERE x = ?" 1]
           (sut/format {:delete-from :foo :where [:= :x 1]} {:checking :strict})))
    (is (= ["DELETE foo WHERE x = ?" 1]
           (sut/format {:delete :foo :where [:= :x 1]} {:checking :strict})))
    (is (= ["UPDATE foo SET x = ? WHERE x = ?" 1 1]
           (sut/format {:update :foo :set {:x 1} :where [:= :x 1]} {:checking :strict})))
    (is (= ["DELETE FROM foo WHERE x = ?" 1]
           (sut/format '{delete-from foo where (= x 1)} {:checking :strict})))
    (is (= ["DELETE foo WHERE x = ?" 1]
           (sut/format '{delete foo where (= x 1)} {:checking :strict})))
  ...
  (testing "empty WHERE clauses flagged in basic mode"
    (is (thrown-with-msg? ExceptionInfo #"without a non-empty"
                          (sut/format {:delete-from :foo} {:checking :basic})))
    (is (thrown-with-msg? ExceptionInfo #"without a non-empty"
                          (sut/format {:delete :foo} {:checking :basic})))
    (is (thrown-with-msg? ExceptionInfo #"without a non-empty"
                          (sut/format {:update :foo :set {:x 1}} {:checking :basic})))
    (is (thrown-with-msg? ExceptionInfo #"without a non-empty"
                          (sut/format '{delete-from foo} {:checking :basic})))
    (is (thrown-with-msg? ExceptionInfo #"without a non-empty"
                          (sut/format '{delete foo} {:checking :basic})))
...
(deftest quoting-:%-syntax
  (testing "quoting of expressions in functions shouldn't depend on syntax"
    (is (= ["SELECT SYSDATE()"]
           (sut/format {:select [[[:sysdate]]]})
           (sut/format {:select :%sysdate})))
    (is (= ["SELECT COUNT(*)"]
           (sut/format {:select [[[:count :*]]]})
           (sut/format {:select :%count.*})))
    (is (= ["SELECT AVERAGE(`foo-foo`)"]
           (sut/format {:select [[[:average :foo-foo]]]} :dialect :mysql)
           (sut/format {:select :%average.foo-foo} :dialect :mysql)))
    (is (= ["SELECT GREATER(`foo-foo`, `bar-bar`)"]
           (sut/format {:select [[[:greater :foo-foo :bar-bar]]]} :dialect :mysql)
           (sut/format {:select :%greater.foo-foo.bar-bar} :dialect :mysql)))
    (is (= ["SELECT MIXED_KEBAB(`yum-yum`)"]
           (sut/format {:select :%mixed-kebab.yum-yum} :dialect :mysql)))
    (is (= ["SELECT MIXED_KEBAB(`yum_yum`)"]
           (sut/format {:select :%mixed-kebab.yum-yum} :dialect :mysql :quoted-snake true)))
    ;; qualifier is always - -> _ converted:
    (is (= ["SELECT MIXED_KEBAB(`yum_yum`.`bar-bar`, `a_b`.`c-d`)"]
           (sut/format {:select (keyword "%mixed-kebab.yum-yum/bar-bar.a-b/c-d")} :dialect :mysql)))
    ;; name is only - -> _ converted when snake_case requested:
    (is (= ["SELECT MIXED_KEBAB(`yum_yum`.`bar_bar`, `a_b`.`c_d`)"]
           (sut/format {:select (keyword "%mixed-kebab.yum-yum/bar-bar.a-b/c-d")} :dialect :mysql :quoted-snake true)))
    (is (= ["SELECT RANSOM(`NoTe`)"]
           (sut/format {:select [[[:ransom :NoTe]]]} :dialect :mysql)
           (sut/format {:select :%ransom.NoTe} :dialect :mysql))))
  (testing "issue 352: literal function calls"
    ...
    (is (= ["SELECT count(*)"]
           (sut/format {:select [[[:'count :*]]]})))
    (is (= ["SELECT Mixed_Kebab(yum_yum)"]
           (sut/format {:select [[[:'Mixed-Kebab :yum-yum]]]})))
    (is (= ["SELECT `Mixed-Kebab`(`yum-yum`)"]
           (sut/format {:select [[[:'Mixed-Kebab :yum-yum]]]} :dialect :mysql)))
    (is (= ["SELECT other_project.other_dataset.other_function(?, ?)" 1 2]
           (sut/format {:select [[[:'other-project.other_dataset.other_function 1 2]]]})))
    (is (= ["SELECT \"other-project\".\"other_dataset\".\"other_function\"(?, ?)" 1 2]
           (sut/format {:select [[[:'other-project.other_dataset.other_function 1 2]]]} :dialect :ansi)))))
...
(deftest fetch-offset-issue-338
  (testing "default offset (with and without limit)"
    (is (= ["SELECT foo FROM bar LIMIT ? OFFSET ?" 10 20]
           (sut/format {:select :foo :from :bar
                        :limit 10 :offset 20})))
